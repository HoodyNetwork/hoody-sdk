...
// Handlers can opt into backpressure by returning a Promise. The session
// defers the per-frame inbound WINDOW replenishment until that Promise
// resolves, so a slow local sink slows down the peer instead of letting
// Node buffer the bytes in memory.
...
export class TunnelSession {
  ...
  private wsAll: (WebSocket | null)[] = [];
  /** Per-stream WS pin: stream_id → ws that delivered STREAM_OPEN. */
  private streamWs = new Map<number, WebSocket>();
  ...
  private wsBatch = new Map<WebSocket, { pending: Uint8Array[]; bytes: number; scheduled: boolean }>();
  private sessionId: string = "";
  private lastHello: HelloResult | null = null;
  private nextBindRef = 1;
  private bindPromises = new Map<number, { resolve: (r: BindResult) => void; reject: (e: Error) => void }>();
  ...
  private rejectAllBindPromises(err: Error) {
    for (const p of this.bindPromises.values()) {
      try { p.reject(err); } catch {}
    }
    this.bindPromises.clear();
  }
  private streamHandlers = new Map<number, FrameHandler>();
   ...
   *  WINDOW(0, n). */
  private sessionCredit: CreditGate | null = null;
  ...
  private streamCredit = new Map<number, CreditGate>();
  /** Cached initial windows from HELLO_OK, used to lazily create per-stream
   *  gates when the SDK first sends to a stream. */
  private initialStreamWindow = 1_048_576;
  private initialSessionWindow = 16_777_216;
  ...
  private closedStreams = new Set<number>();
  private static readonly maxClosedStreams = 4096;
  ...
  private pendingSecondaries = new Set<WebSocket>();
  ...
  async connect(): Promise<string> {
    ...
    await new Promise<string>((resolve, reject) => {
      ...
      resolve = (v: string | PromiseLike<string>) => { clearTimeout(helloTimeoutId); origResolve(v); };
    ...
    if (useV2 && this.lastHello?.joinTickets && this.lastHello.joinTickets.length > 0) {
      try {
        await Promise.all(
          this.lastHello.joinTickets.map((t) => this.openSecondary(t, myGeneration)),
        );
      } catch (err) {
  ...
  private async openSecondary(ticket: JoinTicket, myGeneration: number): Promise<void> {
    return new Promise<void>((resolve, reject) => {
        ...
        for (const frame of result.frames) {
          if (!attached) {
            ...
            if (frame.header.frameType === FrameType.JoinErr) {
              ...
              let e: any = {};
              try { e = JSON.parse(new TextDecoder().decode(frame.payload)); } catch { /* treat as empty */ }
      ...
      ws.onclose = () => {
        ...
        if (this.wsAll[ticket.index] === ws) this.wsAll[ticket.index] = null;
        ...
        const orphanedStreams: number[] = [];
        for (const [sid, pinned] of this.streamWs) {
          if (pinned === ws) orphanedStreams.push(sid);
        }
  ...
  dispatchFrame(frame: Frame, deliveredOn?: WebSocket) {
  ...
  private handleFrame(frame: Frame, deliveredOn?: WebSocket) {
    ...
    if (deliveredOn && frame.header.streamId !== 0) {
      ...
      if (frame.header.frameType === FrameType.StreamOpen) {
        ...
        frame.header.frameType === FrameType.Eof ||
    ...
    switch (frame.header.frameType) {
      case FrameType.BindOk: {
        ...
        // clientBindRef.
        let payload: any;
        try { payload = JSON.parse(new TextDecoder().decode(frame.payload)); } catch {
          this.rejectAllBindPromises(new Error("BIND_OK: malformed payload"));
          break;
        }
        const ref_ = payload.clientBindRef as number;
      ...
      case FrameType.Ping: {
        ...
        const streamId = (frame.header as any).stream_id ?? frame.header.streamId;
        ...
        try {
          if (handler) {
            ...
            if (isDataFrame && ret && typeof (ret as Promise<void>).then === "function") {
  ...
  private sendWindow(streamId: number, increment: number) {
   ...
   */
  async bind(opts: BindOptions): Promise<BindResult> {
    ...
    if (opts.label !== undefined) payloadObj.label = opts.label;
    if (opts.takeover !== undefined) payloadObj.takeover = opts.takeover;
    if (opts.host !== undefined) payloadObj.host = opts.host;
    ...
    return new Promise((resolve, reject) => {
      ...
      const wrappedResolve = (r: BindResult) => { clearTimeout(bindTimeoutId); resolve(r); };
      const wrappedReject = (e: Error) => { clearTimeout(bindTimeoutId); reject(e); };
  ...
  onStream(streamId: number, handler: FrameHandler) {
    this.streamHandlers.set(streamId, handler);
  }

  /** Remove stream handler */
  offStream(streamId: number) {
    this.streamHandlers.delete(streamId);
  }
  ...
  private _sendFrameStrict(frame: Frame) {
    const sid = frame.header.streamId;
    const target: WebSocket | null = sid !== 0
      ? (this.streamWs.get(sid) ?? this.ws)
      : this.ws;
    if (!target || target.readyState !== WebSocket.OPEN) {
      throw new Error(
        `sendData: target WebSocket not open (readyState=${target?.readyState ?? "null"})`
      );
    }
  ...
  private flushBatch(ws: WebSocket) {
    const batch = this.wsBatch.get(ws);
    if (!batch || batch.pending.length === 0) return;
    ...
    try {
      ...
      if (ws === this.ws) {
        ...
        for (const [, gate] of this.streamCredit) gate.close(err);
      } else {
        let dataBytes = 0;
        let off = 0;
        while (off + 9 <= payload.length) {
          ...
          const length =
            ...
            (payload[off + 7]! << 8) |
            payload[off + 8]!;
          if (frameType === FrameType.Data) dataBytes += length;
          off += 9 + length;
        }
        if (dataBytes > 0) this.sessionCredit?.release(dataBytes);
  ...
  async expose(port?: number | null, opts?: { label?: string; takeover?: boolean }): Promise<BindResult> {
    return this.bind({ kind: "http", mode: "expose", containerPort: port, ...opts } as BindOptions);
  }

  /** Bind a PULL port. Omit `port` for an ephemeral kernel-assigned port. */
  async pull(port?: number | null, opts?: { label?: string; host?: string }): Promise<BindResult> {
    return this.bind({ kind: "tcp", mode: "pull", containerPort: port, ...opts } as BindOptions);
  }

  /** Send a STREAM_RESPONSE_HEAD */
  sendResponseHead(streamId: number, status: number, headers: [string, string][], final_ = true) {
  ...
  async sendData(streamId: number, data: Uint8Array): Promise<void> {
    ...
    try {
      ...
      if (streamAcquired) streamGate.release(data.length);
      if (sessionAcquired) sessionCredit.release(data.length);
      throw e;
    }
  }
  ...
  private markStreamClosed(streamId: number): void {
    if (this.closedStreams.has(streamId)) return;
    if (this.closedStreams.size >= TunnelSession.maxClosedStreams) {
      const oldest = this.closedStreams.values().next().value;
      if (oldest !== undefined) this.closedStreams.delete(oldest);
    }
  ...
  async close() {
    ...
    if (this.connectingWs && this.connectingWs.readyState !== WebSocket.CLOSED) {
      try { this.connectingWs.close(); } catch {}
    }
    this.connectingWs = null;
      ...
      if (batch.pending.length > 0 && ws.readyState === WebSocket.OPEN) {
        try {
          if (batch.pending.length === 1) {
            ...
            let off = 0;
            for (const f of batch.pending) { out.set(f, off); off += f.length; }
            ws.send(out);
          }
        } catch {}
        batch.pending = [];
  ...
  private resetSessionState(reason: Error) {
    ...
    this.streamCredit.clear();
    for (const [, pending] of this.bindPromises) pending.reject(reason);
    this.bindPromises.clear();
    for (const w of this.wsAll) {
