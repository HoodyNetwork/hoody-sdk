...
export function createProxyAuthMiddleware(
  getAuth: () => ProxyAuth | ProxyAuthPolicy | undefined,
  baseURL: string
  ...
  function isKitUrl(url: string): boolean {
    try {
  ...
  const middleware = {
    _proxyAuthMiddleware: true as const,
    onRequest(ctx: IHttpClientMiddlewareRequestContext) {
      ...
      const requestAuth = ctx.middlewareContext?.kitAuth as ProxyAuth | undefined;
      
      let resolvedAuth: ProxyAuth | undefined;
      const auth = getAuth();

      if (requestAuth) {
        resolvedAuth = requestAuth;
      } else if (auth && isProxyAuthPolicy(auth)) {
        const namespace = ctx.middlewareContext?._kitNamespace as string | undefined;
        resolvedAuth = (namespace && auth.services?.[namespace as any]) || auth.default;
      } else if (auth) {
        resolvedAuth = auth;
      }
