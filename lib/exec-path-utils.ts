...
export function escapeLonePercents(path: string): string {
 ...
 */
export function decodePathRepeatedly(path: string, helperName: string): string {
  let decoded = path;
 ...
 */
export function validatePathCommon(normalized: string, decoded: string, helperName: string): void {
  ...
  const hasTraversal = (value: string): boolean => value.split('/').filter(Boolean).includes('..');
 ...
 */
export function assertBasePath(path: string, helperName: string): string {
