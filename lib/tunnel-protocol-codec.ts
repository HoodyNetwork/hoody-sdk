...
const KNOWN_FRAME_TYPES: Set<number> = new Set(
...
export function encodeFrame(frame: Frame): Uint8Array {
  if (frame.payload.length > MAX_PAYLOAD_SIZE) {
    throw new CodecError(
      `payload exceeds max size: ${frame.payload.length} > ${MAX_PAYLOAD_SIZE}`,
      "PAYLOAD_TOO_LARGE"
    );
  }
...
export function encodeFrames(frames: Frame[]): Uint8Array {
  let total = 0;
  for (const f of frames) total += HEADER_SIZE + f.payload.length;
  const buf = new Uint8Array(total);
  const view = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
  let off = 0;
...
export function decodeFrame(data: Uint8Array): Frame | null {
  if (data.length < HEADER_SIZE) {
    throw new CodecError(
      `frame too short: ${data.length} bytes (minimum ${HEADER_SIZE})`,
      "TOO_SHORT"
    );
  }
  ...
  if (length > MAX_PAYLOAD_SIZE) {
    throw new CodecError(
      `payload exceeds max size: ${length} > ${MAX_PAYLOAD_SIZE}`,
      "PAYLOAD_TOO_LARGE"
    );
  }
  ...
  if (data.length < payloadEnd) {
    throw new CodecError(
      `frame truncated: declared ${length} bytes but only ${data.length - HEADER_SIZE} available`,
      "TRUNCATED"
    );
  }
  ...
  if (!KNOWN_FRAME_TYPES.has(typeByte)) {
    ...
    if (isMandatoryUnknown(typeByte)) {
      throw new CodecError(
        `unknown mandatory frame type: 0x${typeByte.toString(16).padStart(2, "0")}`,
        "UNKNOWN_MANDATORY_TYPE"
      );
    }
    throw new CodecError(
      `unknown frame type: 0x${typeByte.toString(16).padStart(2, "0")}`,
      "UNKNOWN_TYPE"
    );
  }
  ...
  return {
    header: {
      frameType: typeByte as FrameType,
      streamId,
      length,
    },
    payload,
 ...
 */
export function decodeFrames(data: Uint8Array): DecodeResult {
  if (data.length === 0) {
    throw new CodecError("empty WS message", "EMPTY_MESSAGE");
  }
  if (data.length > MAX_MESSAGE_SIZE) {
    throw new CodecError(
      `WS message too large: ${data.length} > ${MAX_MESSAGE_SIZE}`,
      "MESSAGE_TOO_LARGE"
    );
  }
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  ...
  while (offset < data.length) {
    ...
    if (rawFrameCount > MAX_FRAMES_PER_MESSAGE) {
      throw new CodecError(
        `too many frames: ${rawFrameCount} > ${MAX_FRAMES_PER_MESSAGE}`,
        "TOO_MANY_FRAMES"
      );
    }
    if (offset + HEADER_SIZE > data.length) {
      throw new CodecError(
        `frame too short: ${data.length - offset} bytes (minimum ${HEADER_SIZE})`,
        "TOO_SHORT"
      );
    }
    ...
    if (payloadEnd > data.length) {
      throw new CodecError(
        `frame truncated at offset ${offset}: declared ${length} bytes`,
        "TRUNCATED"
      );
    }
    if (KNOWN_FRAME_TYPES.has(typeByte)) {
      frames.push({
        header: { frameType: typeByte as FrameType, streamId, length },
...
export function dataFrame(streamId: number, payload: Uint8Array): Frame {
  return {
...
export function pingFrame(nonce: bigint): Frame {
...
export function pongFrame(nonce: bigint): Frame {
  const payload = new Uint8Array(8);
  new DataView(payload.buffer).setBigUint64(0, nonce, false);
  return {
...
export function windowFrame(streamId: number, increment: number): Frame {
...
export function eofFrame(streamId: number): Frame {
  return {
