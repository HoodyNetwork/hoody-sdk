...
export type HoodySignatureHeaderCarrier =
  | Headers
  | Response
...
function getHeaderFromRecord(record: Record<string, unknown>, headerName: string): string | undefined {
 ...
 */
export function getHoodySignatureHeader(source: HoodySignatureHeaderCarrier): string | undefined {
  ...
  if (source && typeof source === 'object' && 'headers' in source) {
    const nested = (source as { headers?: Headers | Record<string, unknown> }).headers;
    if (!nested) return undefined;
    return getHoodySignatureHeader(nested as HoodySignatureHeaderCarrier);
  }

  if (source && typeof source === 'object') {
    return getHeaderFromRecord(source as Record<string, unknown>, 'x-hoody-signature');
  }

  return undefined;
}

function parsePairs(rawHeader: string): Record<string, string> {
  ...
  while (i < rawHeader.length) {
    ...
    while (i < rawHeader.length && (rawHeader[i] === ' ' || rawHeader[i] === '\t' || rawHeader[i] === ',')) i++;
    if (i >= rawHeader.length) break;

    // Read key up to '='
    const keyStart = i;
    while (i < rawHeader.length && rawHeader[i] !== '=' && rawHeader[i] !== ',') i++;
    if (i >= rawHeader.length || rawHeader[i] !== '=') {
      // No '=' for this token; skip it
      while (i < rawHeader.length && rawHeader[i] !== ',') i++;
      continue;
    }
    ...
    if (rawHeader[i] === '"') {
      ...
      while (i < rawHeader.length && rawHeader[i] !== '"') {
        if (rawHeader[i] === '\\' && i + 1 < rawHeader.length) i++;
        i++;
      }
      value = rawHeader.slice(vStart, i);
      if (i < rawHeader.length) i++; // skip closing quote
      // Skip to next comma
      while (i < rawHeader.length && rawHeader[i] !== ',') i++;
    } else {
      const vStart = i;
 ...
 */
export function parseHoodySignatureHeader(rawHeader: string): HoodySignatureHeader {
  ...
  if (!t) throw new Error('X-Hoody-Signature missing "t"');
  if (!kid) throw new Error('X-Hoody-Signature missing "kid"');
  if (!path) throw new Error('X-Hoody-Signature missing "path"');
  if (!sig) throw new Error('X-Hoody-Signature missing "sig"');
 ...
 */
export function parseHoodySignatureFrom(source: HoodySignatureHeaderCarrier): HoodySignatureHeader | undefined {
 ...
 */
export function verifyHoodySignatureHeader(
  header: HoodySignatureHeader,
  input: VerifyHoodySignatureInput,
  ...
  if (header.method === undefined) return false;
  if (header.statusCode === undefined) return false;
  ...
  if (header.method !== method.toUpperCase()) return false;
  if (header.statusCode !== statusCode) return false;
  if (header.path !== path) return false;

  // Optional kid allowlist (key rotation pinning).
  if (options.allowedKeyIds !== undefined) {
    const set =
      options.allowedKeyIds instanceof Set
        ? (options.allowedKeyIds as ReadonlySet<string>)
        : new Set<string>(options.allowedKeyIds as readonly string[]);
    if (!set.has(header.keyId)) return false;
  }
  ...
  if (Number.isFinite(skew)) {
    ...
    if (Math.abs(now - header.timestamp) > skew) return false;
  }
 ...
 */
export function verifyHoodySignatureFrom(
  source: HoodySignatureHeaderCarrier,
  input: VerifyHoodySignatureInput,
  options: VerifyHoodySignatureOptions = {},
): boolean {
  let parsed: HoodySignatureHeader;
  try {
