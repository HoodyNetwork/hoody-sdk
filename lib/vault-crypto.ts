...
export class VaultCryptoError extends Error {
  public readonly kind: 'invalid-envelope' | 'unsupported-version' | 'invalid-kdf' | 'decrypt-failed';
  constructor(kind: VaultCryptoError['kind'], message: string, options?: { cause?: unknown }) {
...
function loadNodeCrypto():
  ...
  try {
    ...
    const loader = Function('try { return require; } catch { return null; }')() as
      | ((id: string) => unknown)
      | null;

    if (!loader) return null;
    return loader('node:crypto') as {
      webcrypto?: Crypto;
      randomBytes?: (size: number) => Uint8Array | Buffer;
      randomFillSync?: (buffer: Uint8Array) => Uint8Array;
    };
 ...
 */
function fillRandomBytes(buffer: Uint8Array): RandomSource {
  ...
  if (!insecureRandomWarningShown && typeof console !== 'undefined' && typeof console.warn === 'function') {
    console.warn(
      '[hoody-sdk] vault-crypto is using pseudo-random fallback (no system CSPRNG available). ' +
...
function encodeBigInt64(value: bigint): Uint8Array {
 ...
 */
function collectBestEffortEntropy(): Uint8Array {
 ...
 */
function fillPseudoRandomBytes(buffer: Uint8Array): void {
 ...
 */
function deriveKeyNoble(password: string, salt: Uint8Array, iterations: number): Uint8Array {
...
function inferRngSource(...sources: RandomSource[]): RandomSource {
  if (sources.includes('pseudo-random')) return 'pseudo-random';
  if (sources.includes('node-crypto')) return 'node-crypto';
  return 'webcrypto';
}
...
export async function encrypt(plaintext: string, password: string): Promise<string> {
  ...
  let backend: CryptoBackend = 'noble-js';
 ...
 */
export async function decrypt(envelopeStr: string, password: string): Promise<string> {
...
export function parseEnvelope(value: string): EncryptedEnvelope {
  ...
  if (envelope === null || typeof envelope !== 'object' || typeof (envelope as { _enc?: unknown })._enc !== 'number') {
