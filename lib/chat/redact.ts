...
export function redactArgv(argv: readonly string[]): string[] {
  return argv.map((arg, i) => {
    if (i > 0 && (SENSITIVE_FLAGS as Set<string>).has(argv[i - 1]!)) return REDACTED;
    const eq = arg.indexOf('=');
    if (eq > 0 && (SENSITIVE_FLAGS as Set<string>).has(arg.slice(0, eq))) {
...
export function redactForDisk(text: string): string {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if ((SENSITIVE_FLAGS as Set<string>).has(trimmed)) {
    ...
    if (eq > 0 && (SENSITIVE_FLAGS as Set<string>).has(trimmed.slice(0, eq))) {
