...
export function acceptFilePath(): string {
 ...
 */
export async function readAcceptFile(): Promise<AcceptFile> {
  ...
  try {
    ...
    if (s.isSymbolicLink()) {
      if (process.stderr.isTTY) {
        process.stderr.write(
          `[hoody chat] Refusing to read accept file through a symlink: ${path}. ` +
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && 'origins' in (parsed as any) &&
          (parsed as any).origins !== null &&
          typeof (parsed as any).origins === 'object') {
        return parsed as AcceptFile;
      }
      ...
      try {
        ...
        if (process.stderr.isTTY) {
          process.stderr.write(
            `[hoody chat] Accept file corrupted; quarantined to ${quarantine}. ` +
 ...
 */
async function writeAcceptFile(file: AcceptFile): Promise<void> {
  ...
  try {
    ...
    if (s.isSymbolicLink()) {
      throw new Error(
        `Refusing to write accept file through a symlink: ${path}. ` +
  ...
  try {
    ...
    for (const name of entries) {
      if (!name.startsWith('chat-accept.json.tmp-')) continue;
  ...
  try {
    ...
    if (!renamed) {
      ...
      try { await unlink(tmp); } catch { /* nothing we can do */ }
    }
  }
}

export type AcceptanceStatus =
  | { status: 'ok'; origin: string; reason: 'builtin' | 'local' | 'file' | 'flag' | 'env' | 'prompt' }
  | { status: 'needs-tty-prompt'; origin: string }
  | { status: 'refused'; origin: string; reason: string };
...
export async function checkAcceptance(
  ...
  if (!opts.sessionOnly) {
    ...
    if (file.origins[origin]) return { status: 'ok', origin, reason: 'file' };
  }

  if (opts.flagValue && matchesOrigin(opts.flagValue, origin)) {
    if (!opts.sessionOnly) await recordAcceptance(origin, 'flag');
    return { status: 'ok', origin, reason: 'flag' };
  }
  if (opts.envValue && matchesOrigin(opts.envValue, origin)) {
    if (!opts.sessionOnly) await recordAcceptance(origin, 'env');
...
export async function confirmAcceptance(origin: string): Promise<AcceptanceStatus> {
 ...
 */
function matchesOrigin(input: string, normalized: string): boolean {
  try {
...
async function recordAcceptance(
  ...
  try {
    ...
    process.stderr.write(
      `[hoody chat] Could not persist endpoint acceptance for ${origin} (${msg}); accepted for this session only.\n`,
    );
  }
}
