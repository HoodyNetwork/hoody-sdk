 ...
 */
export function createRenderer(opts: RendererOptions = {}): StreamingRenderer {
  ...
  if (raw) return new RawRenderer(out);
...
class RawRenderer implements StreamingRenderer {
  ...
  write(delta: string): void {
    ...
    if (!this.lastChunkEndedWithNewline) this.out.write('\n');
  }
}
...
interface FenceState {
  lang: string;
  /** Accumulated body content (after the opening fence, before the close). */
  buf: string;
  /** When the fence was opened (ms). Used for idle-flush calc. */
  openedAt: number;
  /** ms of the last append. */
  lastAppendAt: number;
  /** Idle-flush timer handle (or undefined). */
  idleTimer: ReturnType<typeof setTimeout> | undefined;
  /** True if a defensive flush has already emitted partial content — we must
   *  render the tail when the fence finally closes. */
  partialFlushed: boolean;
}

class AnsiRenderer implements StreamingRenderer {
  ...
  private proseLine = '';
  private lastProseRenderAt = 0;
  private proseThrottleTimer: ReturnType<typeof setTimeout> | undefined;
  /** Current fence (open) or undefined (prose mode). */
  private fence: FenceState | undefined;
  /** Whether the most-recent character written to `out` was `\n`. Used to
   *  ensure exactly one trailing newline. */
  private lastWrittenIsNewline = true;

  constructor(private out: NodeJS.WritableStream, private extraDanger: RegExp[]) {}
  ...
  write(delta: string): void {
    ...
    if (!this.lastWrittenIsNewline) this.emit('\n');
  ...
  // Prose rendering
  // -------------------------------------------------------------------------
  private flushProseLine(trailingNewline: boolean): void {
    if (this.proseThrottleTimer) {
      clearTimeout(this.proseThrottleTimer);
      this.proseThrottleTimer = undefined;
    }
    ...
    if (trailingNewline) this.emit('\n');
  }

  private scheduleProseRender(): void {
    if (this.proseThrottleTimer) return;
    const now = Date.now();
  ...
  // Fence rendering
  // -------------------------------------------------------------------------
  private renderClosedFence(): void {
  ...
  private maybeDefensiveFlush(): void {
    if (!this.fence) return;
    if (this.fence.buf.length < FENCE_MAX_BUF_BYTES) return;
    this.flushFenceAsPartial('buffer size limit reached');
  }

  private scheduleFenceIdleTimer(): void {
    if (!this.fence) return;
    if (this.fence.idleTimer) return;
  ...
  private clearFenceIdleTimer(): void {
  ...
  private flushFenceAsPartial(reason: string): void {
    if (!this.fence) return;
...
// ---------------------------------------------------------------------------
// Fence detection (pure functions, exported for testing)
// ---------------------------------------------------------------------------

export interface OpeningFence {
  index: number;
  length: number; // characters consumed including the trailing newline after the fence line
  lang: string;
}
 ...
 */
export function findOpeningFence(s: string): OpeningFence | -1 {
  let from = 0;
  while (from < s.length) {
 ...
 */
export function findClosingFence(s: string): ClosingFence | -1 {
  let from = 0;
  while (from < s.length) {
    const i = s.indexOf('```', from);
    if (i === -1) return -1;
...
export function formatInline(s: string): string {
  ...
  out = out.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_m, label, url) =>
    `${chalk.underline.cyan(label)} ${chalk.dim(`(${url})`)}`,
  );
...
// ---------------------------------------------------------------------------
// Fence rendering (boxed output with gutter + header + footer)
// ---------------------------------------------------------------------------
...
export function renderFenceBlock(
  out: NodeJS.WritableStream,
  lang: string,
  body: string,
  classification: ClassificationResult,
  extras: RenderFenceExtras = {},
): void {
  const isShell = classification.isShellFence;
  const gutter = (() => {
    if (!isShell) return chalk.dim(GUTTER);
    if (classification.severity === 'danger') return chalk.red.bold(GUTTER);
    if (classification.severity === 'review') return chalk.yellow(GUTTER);
