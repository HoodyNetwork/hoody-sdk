...
import { RollingRateLimiter, DEFAULT_CLIENT_RATE_LIMIT, type DocsSearchResult } from './docs-search-tool.js';
import { DedupeCache } from './trigger-parse.js';

function buildLimiter(): RollingRateLimiter {
  return new RollingRateLimiter(
    Number(process.env.HOODY_CHAT_DOCS_RATE_LIMIT) || DEFAULT_CLIENT_RATE_LIMIT,
  );
}

export let docsLimiter: RollingRateLimiter = buildLimiter();
export let triggerDedupe: DedupeCache<DocsSearchResult> = new DedupeCache<DocsSearchResult>();

/** Test-only: rebuild both singletons so HOODY_CHAT_DOCS_RATE_LIMIT changes take effect. */
export function _resetDocsSingletonsForTests(): void {
  docsLimiter = buildLimiter();
  triggerDedupe = new DedupeCache<DocsSearchResult>();
}
