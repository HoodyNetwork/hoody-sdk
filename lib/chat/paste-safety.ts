 ...
 */
export const DANGER_PATTERNS: readonly RegExp[] = Object.freeze([
  // rm -rf / or rm -rf ~
  /\brm\s+-rf?\s+[\/~](\s|$)/,
 ...
 */
export const REVIEW_PATTERNS: readonly RegExp[] = Object.freeze([
 ...
 */
export function parseExtraPatterns(envValue: string | undefined): RegExp[] {
  if (!envValue) return [];
  const out: RegExp[] = [];
  for (const raw of envValue.split(':')) {
    ...
    if (trimmed.length > MAX_EXTRA_PATTERN_SOURCE_CHARS) {
      process.stderr.write(
        `[hoody chat] Skipping pattern in HOODY_CHAT_EXTRA_DESTRUCTIVE_PATTERNS: exceeds ${MAX_EXTRA_PATTERN_SOURCE_CHARS} chars.\n`,
      );
      continue;
    }
    if (looksCatastrophic(trimmed)) {
      process.stderr.write(
        `[hoody chat] Skipping pattern in HOODY_CHAT_EXTRA_DESTRUCTIVE_PATTERNS (nested quantifier / ReDoS risk): ${trimmed}\n`,
      );
      continue;
    }
    try {
      ...
      process.stderr.write(
        `[hoody chat] Invalid regex in HOODY_CHAT_EXTRA_DESTRUCTIVE_PATTERNS: ${trimmed} (${(e as Error).message})\n`,
      );
    }
  }
  return out;
 ...
 */
export function classifyShellBlock(opts: ClassifyOptions): ClassificationResult {
  ...
  const dangerLines: number[] = [];
  const reviewLines: number[] = [];
  ...
  const severity: PasteSeverity =
    dangerLines.length > 0 ? 'danger' : reviewLines.length > 0 ? 'review' : 'none';
