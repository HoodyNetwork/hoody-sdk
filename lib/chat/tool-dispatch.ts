...
import type { InboundToolCall, StreamOptions } from '../ai/openai-client.js';
import {
  executeDocsSearch,
  validateToolArgs,
 ...
 */
export async function dispatchTurn(opts: DispatchOptions): Promise<void> {
  ...
  while (true) {
    ...
    const streamOpts: StreamOptions = {
      ...
      onDone: () => { /* overridden below via `done` promise */ },
    };
    ...
    if (allowed.length === 0) {
      ...
      if (
        ...
        opts.onDelta(
          '(The AI provider tried to call a tool while tools were disabled. ' +
    ...
    if (toolCallsThisTurn >= MAX_TOOL_CALLS_PER_TURN) {
      ...
      for (const tc of assistantToolCalls) {
        messages.push({
          ...
          content: JSON.stringify({
            ...
            message:
              'Tool was already invoked once this turn. Answer from the previously returned context.',
          }),
        });
      }
      ...
      await streamCompletion({
        ...
        signal: opts.signal,
        fetchImpl: opts.fetchImpl,
        onDelta: d => { if (d.content) opts.onDelta(d.content); },
        onDone: () => { /* no-op; streamCompletion resolves when done */ },
      });
      return;
    }
    ...
    const argsJson =
      typeof tc0.function.arguments === 'string' && tc0.function.arguments.length > 0
    ...
    for (const extra of assistantToolCalls.slice(1)) {
      messages.push({
        ...
        content: JSON.stringify({
          error: 'docs-search-max-iterations',
          message:
            'Only the first tool call this turn was executed. Answer from that result.',
        }),
      });
    }
...
async function runStreamOnce(opts: StreamOptions): Promise<{
  text: string;
  toolCalls: InboundToolCall[];
}> {
  let aggregatedText = '';
  let aggregatedToolCalls: InboundToolCall[] = [];
  await streamCompletion({
    ...opts,
    onDelta: d => {
      if (d.content) aggregatedText += d.content;
      opts.onDelta(d);
    },
    onDone: info => {
