...
function strictSemver(v: string): string | null {
...
export interface OracleResult {
  ...
  attempts: Array<{ source: string; error: string }>;
}

/** Extract a `v?<semver>` tag from a validated GitHub release `Location`. */
export function parseTagFromLocation(
  ...
  if (!path.startsWith(want)) return null;
...
function parseNpmLatest(res: FetchedResource): string {
  ...
  try {
    ...
    throw new Error(`npm /latest is not valid JSON: ${(e as Error).message}`);
  }
  const raw = (doc as { version?: unknown })?.version;
...
export async function resolveLatestVersion(opts: OracleOptions): Promise<OracleResult> {
  const attempts: OracleResult['attempts'] = [];
  ...
  const run = (url: string, extra: FetcherOptions = {}): Promise<FetchedResource> => {
    const o = { ...base, ...extra };
    return foreground
  ...
  try {
    ...
    if (!version) {
      throw new Error(
        `github releases/latest did not redirect to a valid tag (status ${res.status}, ` +
  ...
  try {
    ...
    const doc = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(res.body)) as {
      tag_name?: unknown;
    };
