...
function toUint8Array(input: SshPublicKeyInput): Uint8Array {
...
function readSshString(buffer: Uint8Array, offset: number): ReadStringResult {
 ...
 */
function buildEd25519KeyBlob(publicKeyRaw: Uint8Array): Uint8Array {
 ...
 */
function parseEd25519KeyBlob(keyBlob: Uint8Array): Uint8Array {
 ...
 */
function normalizeEd25519PublicKey(input: SshPublicKeyInput): Uint8Array {
  if (typeof input !== 'string') {
  ...
  if (trimmed.startsWith('ssh-')) {
    throw new Error(
      `Unsupported SSH key format "${trimmed.split(/\s+/, 1)[0]}". Expected "${SSH_ED25519_ALGORITHM}".`
    );
  }
...
function normalizeSeed(seed: Ed25519SeedInput): Uint8Array {
...
function generateInsecureSeed(): Uint8Array {
 ...
 */
function createEd25519KeyPair(options: GenerateEd25519SshKeyPairOptions): {
  pair: nacl.SignKeyPair;
  randomSource: GeneratedEd25519SshKeyPair['randomSource'];
} {
  if (options.seed) {
    return {
  ...
  try {
    ...
    throw new Error(
      `Unable to generate key pair without secure randomness: ${message}. ` +
 ...
 */
export function formatEd25519SshPublicKey(publicKey: SshPublicKeyInput, comment?: string): string {
 ...
 */
export function parseEd25519SshPublicKey(publicKey: string): ParsedEd25519SshPublicKey {
  ...
  const parsed: ParsedEd25519SshPublicKey = {
 ...
 */
export function generateEd25519SshKeyPair(
  options: GenerateEd25519SshKeyPairOptions = {}
