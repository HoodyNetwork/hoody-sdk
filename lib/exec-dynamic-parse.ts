...
// ─── Raw entry shape (matches both SDK and CLI response shapes) ──────────────
 ...
 */
export function parseRawScriptEntry(raw: RawScriptEntry): DiscoveredScript[] {
  ...
  if (!isValidToolName(name)) return [];
  ...
  let parameters: DiscoveredParam[] = [];
  ...
  if (schema && typeof schema === 'object') {
    parameters = extractParamsFromSchema(schema as Record<string, unknown>, httpMethod);
    if (isBodyMethod && Object.keys(schema).length > 0) {
      requestBodySchema = schema as Record<string, unknown>;
    }
  }
  ...
  if (Array.isArray(rawMethods) && rawMethods.length > 1) {
    const normalizedMethods = rawMethods
      ...
      .filter((m): m is 'GET' | 'POST' => m === 'GET' || m === 'POST');

    for (const variantMethod of normalizedMethods) {
      if (variantMethod === httpMethod) continue; // primary already added
      const variantName = `${name}-${variantMethod.toLowerCase()}`;
      if (!isValidToolName(variantName)) continue;

      // Re-extract params for the variant method from schema
      let variantParams = schema
        ? extractParamsFromSchema(schema as Record<string, unknown>, variantMethod)
        : [];
 ...
 */
export function parseRawScriptEntries(rawScripts: RawScriptEntry[]): DiscoveredScript[] {
