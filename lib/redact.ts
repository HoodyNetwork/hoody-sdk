...
export function redactHeaders(headers: Record<string, string>): Record<string, string> {
  return Object.fromEntries(
    Object.entries(headers).map(([k, v]) =>
      SECRET_HEADER_RE.test(k) ? [k, PLACEHOLDER] : [k, v],
    ),
  );
}
...
export function redactUrl(url: string): string {
  ...
  try {
    const u = new URL(url);
    if (u.username) u.username = PLACEHOLDER;
    if (u.password) u.password = PLACEHOLDER;
    for (const key of Array.from(u.searchParams.keys())) {
      if (SECRET_FIELD_RE.test(key)) u.searchParams.set(key, PLACEHOLDER);
    }
    ...
    const parts = queryRaw.split('&').map(pair => {
      ...
      try {
        if (SECRET_FIELD_RE.test(decodeURIComponent(k))) return `${k}=${encodeURIComponent(PLACEHOLDER)}`;
 ...
 */
export function redactSensitiveValue(v: unknown, _depth = 0, seen: WeakSet<object> = new WeakSet()): unknown {
  ...
  if (typeof v === 'string') return v.replace(HDY_TOKEN_VALUE_RE, PLACEHOLDER);
  if (typeof v !== 'object') return v;
  if (seen.has(v as object)) return '[Circular]';
  seen.add(v as object);
  if (Array.isArray(v)) return v.map((x) => redactSensitiveValue(x, _depth + 1, seen));
