...
export class EventsManager {
    private ws: ApiConnecteventstreamWebSocket | null = null;
     ...
     */
    private listeners: Map<string, Map<EventCallback, EventFilter | undefined>> = new Map();

    private lifecycleListeners: Map<string, Set<LifecycleCallback>> = new Map();
    private state: 'idle' | 'connecting' | 'connected' | 'reconnecting' | 'disconnecting' = 'idle';
    ...
    constructor(
        private createWsClient: () => Promise<ApiConnecteventstreamWebSocket>,
        options?: {
            autoConnect?: boolean;
    ...
    private async connect(): Promise<void> {
        ...
        try {
            ...
            if (this.ws) {
                ...
                try { this.ws.disconnect('connect-failed'); } catch { }
                this.ws = null;
            }
            if (this.debug) {
     ...
     */
    private routeEvent(event: EventServerMessage): void {
        if (this.debug) {
     ...
     */
    private matchesFilter(event: EventServerMessage, filter: EventFilter | undefined): boolean {
        if (!filter) {
            return true;
        }
     ...
     */
    private emitLifecycleEvent(event: string, ...args: any[]): void {
    ...
    disconnect(): void {
        ...
        if (this.ws) {
            ...
            try { this.ws.removeAllListeners(); } catch { }
            try { this.ws.disconnect('client disconnect'); } catch { }
            this.ws = null;
        }
        this.cleanupWsListeners();
     ...
     */
    private cleanupWsListeners(): void {
