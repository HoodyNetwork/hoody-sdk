...
export interface MountStateFile {
  ...
  auth: { method: 'none' | 'password' | 'token' | 'header' | 'ip' | 'containerClaim' };
  startedAt: string;
...
export function computeMountId(localPath: string, uid: number = userInfo().uid): string {
  const canonical = resolve(localPath);
  return createHash('sha256').update(`${uid}:${canonical}`).digest('hex').slice(0, 16);
}

export function stateFilePath(id: string, home?: string): string {
  return join(getStateDir(home), `${id}.json`);
}

export function configFilePath(id: string, home?: string): string {
  return join(getStateDir(home), `${id}.conf`);
}

async function ensureStateDir(home?: string): Promise<void> {
 ...
 */
export async function claimState(record: MountStateFile, home?: string): Promise<void> {
  ...
  throw new Error(
    `a mount is already registered at ${record.localPath} ` +
...
async function atomicOverwrite(target: string, data: string): Promise<void> {
  const tmp = `${target}.tmp.${process.pid}`;
  await fs.writeFile(tmp, data, { mode: 0o600 });
  await fs.rename(tmp, target);
}

export async function readState(id: string, home?: string): Promise<MountStateFile> {
  const raw = await fs.readFile(stateFilePath(id, home), 'utf-8');
  const parsed = JSON.parse(raw) as MountStateFile;
...
export async function deleteState(id: string, home?: string): Promise<void> {
...
export async function listStateIds(home?: string): Promise<string[]> {
  await ensureStateDir(home);
  let entries: Dirent[];
  try {
...
export async function listStates(home?: string): Promise<MountStateFile[]> {
  const ids = await listStateIds(home);
  const out: MountStateFile[] = [];
  for (const id of ids) {
    try {
 ...
 */
export async function checkLiveness(rec: MountStateFile): Promise<LivenessResult> {
...
async function statWithTimeout(path: string, ms: number): Promise<Stats | null> {
  return new Promise((resolveP, rejectP) => {
    ...
    fs.stat(path).then(
      ...
      (err) => {
        clearTimeout(timer);
        if ((err as NodeJS.ErrnoException).code === 'ENOENT') resolveP(null);
        else rejectP(err);
      },
    );
  });
...
async function isInMountTable(localPath: string): Promise<boolean> {
  const canonical = resolve(localPath);
  try {
    ...
    for (const line of stdout.split('\n')) {
      if (line.includes(` ${canonical} `) || line.endsWith(` ${canonical}`)) return true;
    }
    return false;
  } catch {
    return false;
 ...
 * and unlinking .conf.
 */
export async function pruneStale(home?: string): Promise<{ removed: number; orphans: number }> {
  const ids = await listStateIds(home);
  let removed = 0;
  for (const id of ids) {
    try {
 ...
 */
export async function isMountpointEmpty(localPath: string): Promise<boolean> {
  try {
...
export async function ensureMountpointParent(localPath: string): Promise<void> {
