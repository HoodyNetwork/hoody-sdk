...
export interface HelloServerMessage {
  ...
  features: { buffered: boolean; cache: boolean; outbound_pooling: boolean; streaming: boolean };
  limits: { chunk_bytes: number; idle_timeout_secs: number; max_concurrent_streams: number; max_frame_bytes: number; max_outbound_messages: number; max_queue: number; max_request_bytes: number; stream_timeout_secs: number };
  type: 'hello';
  version: number;
}
...
export class CurlWsRequestChannelWebSocket implements ICurlWsRequestChannelWebSocket {
   ...
   */
  onHello(callback: (message: HelloServerMessage) => void): () => void {
    return this.addEventListener("hello", callback);
  }
   ...
   */
  onPong(callback: (message: PongServerMessage) => void): () => void {
    return this.addEventListener("pong", callback);
  }
   ...
   */
  onAccepted(callback: (message: AcceptedServerMessage) => void): () => void {
    return this.addEventListener("accepted", callback);
  }
   ...
   */
  onCancelled(callback: (message: CancelledServerMessage) => void): () => void {
    return this.addEventListener("cancelled", callback);
  }
   ...
   */
  onResponseStart(callback: (message: ResponseStartServerMessage) => void): () => void {
    return this.addEventListener("response.start", callback);
  }
   ...
   */
  onResponseBody(callback: (message: ResponseBodyServerMessage) => void): () => void {
    return this.addEventListener("response.body", callback);
  }
   ...
   */
  onResponseEnd(callback: (message: ResponseEndServerMessage) => void): () => void {
    return this.addEventListener("response.end", callback);
  }
