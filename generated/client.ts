...
import { HttpClient, IHttpClientConfig, type IRequestData } from './http-client.js';
...
export interface ContainerLike {
  ...
  // withContainer() and getKitUrl() validate id + project_id + a resolvable server
  // at runtime and throw "Invalid container object" when any is missing.
  project_id?: string;
  [key: string]: unknown;
}

function _resolveContainerServer(container: ContainerLike | null | undefined): string | undefined {
  if (!container) return undefined;
  return (container.server_name as string | undefined) ?? (container.server as string | undefined);
}

export class HoodyClient {
  public readonly http: HttpClient;
  private readonly urlTemplates?: Record<string, Record<string, string | number>> | undefined;
  private readonly realmId?: string | undefined;
  private kitAuth?: ProxyAuth | ProxyAuthPolicy | undefined;
  private onKitAuthExpired?: ((namespace: string, error: any) => Promise<ProxyAuth | undefined>) | undefined;
  ...
  public readonly curl: curl.CurlService & {
  ...
  public readonly display: display.DisplayService & {
  ...
  public readonly files: files.FilesService & {
  ...
  public readonly agent: agent.AgentService & {
  ...
  constructor(config: HoodyClientConfig) {
    ...
    const configWithRetry: IHttpClientConfig = {
      ...
      onTokenExpired: async (error: any) => {
    ...
    if (config.kitAuth || config.onKitAuthExpired) {
      const proxyAuthMiddleware = createProxyAuthMiddleware(
        () => (this as any).kitAuth,  // getter — always reads current value
        config.baseURL || ''
      );
    ...
    if (config.onKitAuthExpired) {
      ...
      configWithRetry.onKitAuthExpired = async (ns: string, error: any) => {
        const newAuth = await userCallback(ns, error);
        if (newAuth) (this as any).kitAuth = newAuth;
        return newAuth;
      };
    }
    ...
    this.browser = {
      instances: new browser.InstanceManagementService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      interaction: new browser.BrowserInteractionService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      introspection: new browser.IntrospectionControlService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      health: new browser.ServerHealthMetricsService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      page: new browser.PageContentService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      cookies: new browser.BrowserStateService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      debugging: new browser.DebuggingService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
      history: new browser.BrowsingHistoryService(this.http, 'browser', this.urlTemplates?.['browser'] as any, this.getKitUrlTemplatePattern('browser')),
    };

    this.code = {
      vscode: new code.VscodeService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
      auth: new code.AuthService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
      static: new code.StaticService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
      extensions: new code.ExtensionsService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
      health: new code.HealthService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
      proxy: new code.ProxyService(this.http, 'code', this.urlTemplates?.['code'] as any, this.getKitUrlTemplatePattern('code')),
    };

    this.curl = Object.assign(new curl.CurlService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')), {
      events: new curl.EventsService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      health: new curl.HealthService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      jobs: new curl.JobsService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      schedules: new curl.SchedulesService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      sessions: new curl.SessionsService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      storage: new curl.StorageService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
      ops: new curl.OpsService(this.http, 'curl', this.urlTemplates?.['curl'] as any, this.getKitUrlTemplatePattern('curl')),
    });

    this.daemon = {
      health: new daemon.HealthService(this.http, 'daemon', this.urlTemplates?.['daemon'] as any, this.getKitUrlTemplatePattern('daemon')),
      programs: new daemon.ProgramsService(this.http, 'daemon', this.urlTemplates?.['daemon'] as any, this.getKitUrlTemplatePattern('daemon')),
      control: new daemon.ControlService(this.http, 'daemon', this.urlTemplates?.['daemon'] as any, this.getKitUrlTemplatePattern('daemon')),
      status: new daemon.StatusService(this.http, 'daemon', this.urlTemplates?.['daemon'] as any, this.getKitUrlTemplatePattern('daemon')),
      quickStart: new daemon.QuickStartService(this.http, 'daemon', this.urlTemplates?.['daemon'] as any, this.getKitUrlTemplatePattern('daemon')),
    };

    this.display = Object.assign(new display.DisplayService(this.http, 'display', this.urlTemplates?.['display'] as any, this.getKitUrlTemplatePattern('display')), {
      health: new display.HealthService(this.http, 'display', this.urlTemplates?.['display'] as any, this.getKitUrlTemplatePattern('display')),
      screenshots: new display.ScreenshotsService(this.http, 'display', this.urlTemplates?.['display'] as any, this.getKitUrlTemplatePattern('display')),
      thumbnails: new display.ThumbnailsService(this.http, 'display', this.urlTemplates?.['display'] as any, this.getKitUrlTemplatePattern('display')),
      input: new display.InputService(this.http, 'display', this.urlTemplates?.['display'] as any, this.getKitUrlTemplatePattern('display')),
    });

    this.exec = {
      system: new exec.SystemService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      execution: new exec.ScriptExecutionService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      validate: new exec.ValidateService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      templates: new exec.TemplatesService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      scripts: new exec.ScriptsService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      logs: new exec.LogsService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      cache: new exec.CacheService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      state: new exec.SharedStateService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      route: new exec.RouteService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      monitor: new exec.MonitorService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      health: new exec.HealthService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      dependencies: new exec.DependenciesService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      package: new exec.PackageService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      openapi: new exec.UserOpenapiService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      sdk: new exec.SdkService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      ids: new exec.ListService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      magic: new exec.MagicCommentsService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
      schedules: new exec.SchedulesService(this.http, 'exec', this.urlTemplates?.['exec'] as any, this.getKitUrlTemplatePattern('exec')),
    };

    this.files = Object.assign(new files.FilesService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')), {
      downloads: new files.DownloadsService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      archives: new files.ArchivesService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      backends: new files.BackendsService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      health: new files.HealthService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      journal: new files.JournalService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      mounts: new files.MountsService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      system: new files.SystemService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      images: new files.ImageProcessingService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      webdav: new files.WebdavService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      directories: new files.DirectoriesService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      authentication: new files.AuthenticationService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      ftp: new files.RemoteFtpService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      git: new files.RemoteGitService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      s3: new files.RemoteS3Service(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
      ssh: new files.RemoteSshService(this.http, 'files', this.urlTemplates?.['files'] as any, this.getKitUrlTemplatePattern('files')),
    });

    this.notifications = Object.assign(new notifications.NotificationsService(this.http, 'notifications', this.urlTemplates?.['notifications'] as any, this.getKitUrlTemplatePattern('notifications')), {
      health: new notifications.HealthService(this.http, 'notifications', this.urlTemplates?.['notifications'] as any, this.getKitUrlTemplatePattern('notifications')),
      icons: new notifications.IconsService(this.http, 'notifications', this.urlTemplates?.['notifications'] as any, this.getKitUrlTemplatePattern('notifications')),
      notify: new notifications.NotifyService(this.http, 'notifications', this.urlTemplates?.['notifications'] as any, this.getKitUrlTemplatePattern('notifications')),
    });

    this.sqlite = {
      database: new sqlite.DatabaseService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      health: new sqlite.HealthService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      history: new sqlite.HistoryService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      kvStore: new sqlite.KvStoreService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      sql: new sqlite.SqlService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      docs: new sqlite.DocumentationService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
      query: new sqlite.QueryService(this.http, 'sqlite', this.urlTemplates?.['sqlite'] as any, this.getKitUrlTemplatePattern('sqlite')),
    };
    ...
    this.watch = {
      health: new watch.HealthService(this.http, 'watch', this.urlTemplates?.['watch'] as any, this.getKitUrlTemplatePattern('watch')),
      system: new watch.SystemService(this.http, 'watch', this.urlTemplates?.['watch'] as any, this.getKitUrlTemplatePattern('watch')),
      watchers: new watch.WatchersService(this.http, 'watch', this.urlTemplates?.['watch'] as any, this.getKitUrlTemplatePattern('watch')),
      streams: new watch.StreamsService(this.http, 'watch', this.urlTemplates?.['watch'] as any, this.getKitUrlTemplatePattern('watch')),
    };

    this.cron = {
      crontab: new cron.CrontabService(this.http, 'cron', this.urlTemplates?.['cron'] as any, this.getKitUrlTemplatePattern('cron')),
      health: new cron.HealthService(this.http, 'cron', this.urlTemplates?.['cron'] as any, this.getKitUrlTemplatePattern('cron')),
      system: new cron.SystemService(this.http, 'cron', this.urlTemplates?.['cron'] as any, this.getKitUrlTemplatePattern('cron')),
      entries: new cron.EntriesService(this.http, 'cron', this.urlTemplates?.['cron'] as any, this.getKitUrlTemplatePattern('cron')),
    };

    this.pipe = Object.assign(new pipe.PipeService(this.http, 'pipe', this.urlTemplates?.['pipe'] as any, this.getKitUrlTemplatePattern('pipe')), {
      ui: new pipe.UiService(this.http, 'pipe', this.urlTemplates?.['pipe'] as any, this.getKitUrlTemplatePattern('pipe')),
      info: new pipe.InfoService(this.http, 'pipe', this.urlTemplates?.['pipe'] as any, this.getKitUrlTemplatePattern('pipe')),
      health: new pipe.HealthService(this.http, 'pipe', this.urlTemplates?.['pipe'] as any, this.getKitUrlTemplatePattern('pipe')),
    });

    this.notes = {
      health: new notes.HealthService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      identity: new notes.IdentityService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      sockets: new notes.SocketsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      avatars: new notes.AvatarsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      notebooks: new notes.NotebooksService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      files: new notes.FilesService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      nodes: new notes.NodesService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      documents: new notes.DocumentsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      collaborators: new notes.CollaboratorsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      reactions: new notes.ReactionsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      interactions: new notes.InteractionsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      comments: new notes.CommentsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      versions: new notes.VersionsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      databases: new notes.DatabasesService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      users: new notes.UsersService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
      mutations: new notes.MutationsService(this.http, 'notes', this.urlTemplates?.['notes'] as any, this.getKitUrlTemplatePattern('notes')),
    };
    ...
    this.agent = Object.assign(new agent.AgentService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')), {
      settings: new agent.SettingsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      agents: new agent.AgentsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      discovery: new agent.DiscoveryService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      ...
      loops: new agent.LoopsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      tasks: new agent.TasksService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      tools: new agent.ToolsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      workflows: new agent.WorkflowsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      skills: new agent.SkillsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      statistics: new agent.StatisticsService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
      todos: new agent.TodosService(this.http, 'agent', this.urlTemplates?.['agent'] as any, this.getKitUrlTemplatePattern('agent')),
    });
  }
  ...
  public async getAuthToken(): Promise<string | undefined> {
    const current = (this.http as any).config?.token;
    if (!current && this.credentials) {
      await this.login(this.credentials);
    }
    const token = (this.http as any).config?.token;
   ...
   */
  public async login(credentials: { username: string; password: string }): Promise<ApiAuthenticationLoginResponse> {
    this.credentials = credentials;
    // @ts-ignore - Assuming api namespace exists and has authentication service
    if ((this as any).api && (this as any).api.authentication) {
      // @ts-ignore
      const response = await (this as any).api.authentication.login(credentials) as ApiAuthenticationLoginResponse;
   ...
   */
  private async handleAuthRefresh(error: any): Promise<string | undefined> {
    if (!this.autoRefresh) return undefined;
    if (error?.status !== 401) return undefined;

    // @ts-ignore - Assuming api namespace exists
    if (!(this as any).api || !(this as any).api.authentication) return undefined;

    // Try refresh token first
    if (this.refreshToken) {
      try {
        // @ts-ignore
        const refreshResponse = await (this as any).api.authentication.refreshToken({ refreshToken: this.refreshToken });
    ...
    if (this.credentials) {
      try {
        // @ts-ignore
        const loginResponse = await (this as any).api.authentication.login(this.credentials);
   ...
   */
  private async handleRealmError(error: any): Promise<boolean> {
    if (error.status !== 403) return false;
    const msg = error.response?.message || error.message || '';
    const isRealmError =
      msg.includes('requires a realm-scoped URL') ||
      msg.includes('not valid for realm');
    if (!isRealmError) return false;

    // Fetch allowed realms from base domain (bootstrap exception)
    let allowedRealms: string[] = [];
    try {
      ...
      const baseHttp = new HttpClient({
        ...
        token: (this.http as any).config.token,
        timeout: (this.http as any).config.timeout || 10000,
      });
      const meResult: any = await baseHttp.get('/api/v1/auth/tokens/me', {});
  ...
  async withContainer(
    ...
    if (typeof containerOrId === 'string') {
      ...
      const response = await this.api.containers.get(containerOrId) as any;
    ...
    const config = (this.http as any).config as IHttpClientConfig;
    
    const newConfig: HoodyClientConfig = {
    ...
    if (config.token) newConfig.token = config.token;
    if (config.timeout !== undefined) newConfig.timeout = config.timeout;       // keep timeout:0
    if (config.retries !== undefined) newConfig.retries = config.retries;       // keep retries:0
    if (config.retryDelayMs !== undefined) newConfig.retryDelayMs = config.retryDelayMs;
    if (config.retryOnStatuses) newConfig.retryOnStatuses = config.retryOnStatuses;
    if (config.headers) newConfig.headers = config.headers;
    if (config.cache) newConfig.cache = config.cache;
    if (config.middlewares) newConfig.middlewares = config.middlewares;
    if (config.onTokenExpired) newConfig.onTokenExpired = config.onTokenExpired;
    if (config.refreshToken) newConfig.refreshToken = config.refreshToken;
    if (config.autoRetryAuth !== undefined) newConfig.autoRetryAuth = config.autoRetryAuth;
    if (config.transport) newConfig.transport = config.transport;
    if (config.forceIPv4 !== undefined) newConfig.forceIPv4 = config.forceIPv4;
    if (config.forceIPv4Cache) newConfig.forceIPv4Cache = config.forceIPv4Cache;
    if (config.clientId) newConfig.clientId = config.clientId;
    if (config.clientName) newConfig.clientName = config.clientName;

    if (this.realmId) newConfig.realmId = this.realmId;
    if (this.credentials) newConfig.credentials = this.credentials;
    ...
    if (options?.kitAuth) {
      ...
      if (newConfig.middlewares) {
        newConfig.middlewares = newConfig.middlewares.filter(
          m => !(m as { _proxyAuthMiddleware?: boolean })._proxyAuthMiddleware
        );
      }
  ...
  withRealm(realmId?: string): HoodyClient {
    ...
    const clearRealmScope =
      normalizedRealm.length === 0 ||
      normalizedRealm === '*' ||
    ...
    if (config.token) newConfig.token = config.token;
    if (config.timeout !== undefined) newConfig.timeout = config.timeout;
    if (config.retries !== undefined) newConfig.retries = config.retries;
    if (config.retryDelayMs !== undefined) newConfig.retryDelayMs = config.retryDelayMs;
    if (config.retryOnStatuses) newConfig.retryOnStatuses = config.retryOnStatuses;
    if (config.headers) newConfig.headers = config.headers;
    if (config.cache) newConfig.cache = config.cache;
    ...
    if (config.clientName) newConfig.clientName = config.clientName;
    if (this.credentials) newConfig.credentials = this.credentials;
    if (this.urlTemplates) newConfig.urlTemplates = this.urlTemplates;
    if (this.kitAuth) newConfig.kitAuth = this.kitAuth;
    if (this.onKitAuthExpired) newConfig.onKitAuthExpired = this.onKitAuthExpired;
    newConfig.autoRefresh = this.autoRefresh;
    return new HoodyClient(newConfig);
  }
  ...
  getKitUrl(
    ...
    const hasOptionsObject =
      typeof serviceIndexOrOptions === 'object'
      && serviceIndexOrOptions !== null
      && !Array.isArray(serviceIndexOrOptions);
    const isLocal = hasOptionsObject && (serviceIndexOrOptions as any).local === true;
   ...
   */
  private getKitUrlTemplatePattern(namespace: string): string {
    const containersDomain = this.resolveContainersDomain();
   ...
   */
  private resolveContainersDomain(): string {
    ...
    try {
      ...
      if (isIpv4 || isIpv6 || isLocalhost) return host;
  ...
  private resolveKitServiceSegment(
    kit: string,
    serviceIndexOrOptions: number | { serviceIndex?: number; protocol?: 'http' | 'https'; port?: number; local?: boolean }
  ): string {
    ...
    const hasOptionsObject =
      ...
      && serviceIndexOrOptions !== null
      && !Array.isArray(serviceIndexOrOptions);
    const options = hasOptionsObject ? serviceIndexOrOptions as { serviceIndex?: number; protocol?: 'http' | 'https'; port?: number; local?: boolean } : {};
   ...
   */
  async checkIp(options?: { signal?: AbortSignal; timeoutMs?: number }): Promise<ApiUtilitiesGetIpInfoResponse> {
    const url = `https://${this.resolveIpDomain()}/api/v1/ip`;
    const requestData: IRequestData = {};
    if (options?.signal) requestData.signal = options.signal;
    if (options?.timeoutMs !== undefined) requestData.timeoutMs = options.timeoutMs;
    return this.http.get<ApiUtilitiesGetIpInfoResponse>(url, requestData);
  }
   ...
   */
  private resolveIpDomain(): string {
