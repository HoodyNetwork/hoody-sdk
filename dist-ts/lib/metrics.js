...
const DEFAULT_BYTE_UNIT_SYSTEM = 'iec';
/**
 ...
 */
function almostEqual(a, b, relative = 0.001, absolute = 0.5) {
 ...
 * KiB, MiB, GiB, TiB.
 */
function formatBytes(bytes, unitSystem = DEFAULT_BYTE_UNIT_SYSTEM) {
 ...
 */
function defineCachedGetter(target, key, compute) {
 ...
 */
function extractReferenceNowMs(envelope, data, fallbackNowMs) {
...
function extractNetworkCounters(networkValue) {
 ...
 */
function normalizeCpuInPlace(containerStats, referenceNow) {
    ...
    const looksLikeBrokenCumulativeFormula = usageNs !== undefined
    ...
    if (looksLikeBrokenCumulativeFormula && usageNs !== undefined) {
        const containerId = typeof containerStats.id === 'string' && containerStats.id.trim() !== ''
            ? containerStats.id
            : undefined;
 ...
 */
function normalizeMemoryInPlace(containerStats) {
 ...
 */
function buildContainerHumanMetrics(containerStats, referenceNow) {
    const cpu = asRecord(containerStats.cpu);
    const memory = asRecord(containerStats.memory);
    ...
    const rootPercent = rootUsageBytes !== undefined
        && rootTotalBytes !== undefined
        && rootTotalBytes > 0
        ? (rootUsageBytes / rootTotalBytes) * 100
        : undefined;
    const startedAtMs = parseTimestampMs(containerStats.started_at);
    const uptimeSeconds = startedAtMs !== undefined && referenceNow.nowMs > startedAtMs
    ...
    const timingHuman = {
        ...
        processing_time: typeof containerStats.processing_time === 'string' ? containerStats.processing_time : null,
 ...
 */
function normalizeContainerStatsDataInPlace(containerStats, referenceNow) {
    normalizeCpuInPlace(containerStats, referenceNow);
    normalizeMemoryInPlace(containerStats);
    defineCachedGetter(containerStats, 'metrics_human', () => buildContainerHumanMetrics(containerStats, referenceNow));
}
function buildProjectHumanSummary(stats) {
    let runningCount = 0;
    ...
    const memoryPercent = memoryTotalBytes > 0 ? (memoryUsageTotalBytes / memoryTotalBytes) * 100 : undefined;
    const diskPercent = diskTotalBytes > 0 ? (diskUsageTotalBytes / diskTotalBytes) * 100 : undefined;
    return {
        container_count: stats.length,
        running_container_count: runningCount,
        cpu_usage_percent_total: roundTo(cpuTotalPercent, 4),
        cpu_usage_percent_average: cpuAveragePercent !== undefined ? roundTo(cpuAveragePercent, 4) : null,
        cpu_usage_percent_average_text: cpuAveragePercent !== undefined ? formatPercent(cpuAveragePercent) : null,
 ...
 */
export function normalizeContainerStatsResponse(response) {
 ...
 */
export function normalizeProjectStatsResponse(response) {
    const envelope = asRecord(response);
    if (!envelope)
        return response;
    const originalData = asRecord(envelope.data);
    ...
    if (!Array.isArray(originalStats))
        return response;
    const clonedResponse = cloneValue(response);
    const clonedEnvelope = asRecord(clonedResponse);
    if (!clonedEnvelope)
    ...
    defineCachedGetter(data, 'metrics_human_summary', () => buildProjectHumanSummary(normalizedStats));
    return clonedResponse;
}
