...
const DANGEROUS_RE = /[\\:*<>|"]/;
function rejectChars(value, label) {
...
function rejectTraversalSegments(segments, label) {
...
function rejectTrailingDots(segments, label) {
    for (const seg of segments) {
...
function checkComponentLengths(segments, label) {
    for (const seg of segments) {
 ...
 */
function validateScreenshotPath(raw, label) {
    // 0. Type check
 ...
 */
function resolvePathAndFormat(source, userPath, userDir, userFormat) {
    ...
    if (source === 'display' && userFormat === 'jpeg') {
        throw new ScreenshotSaveError('FORMAT_MISMATCH', 'Display screenshots only support PNG format', { source });
    }
    let resolvedFormat = userFormat ?? 'png';
    let resolvedPath;
    ...
    if (userPath !== undefined) {
        ...
        if (ext && VALID_EXTENSIONS.has(ext)) {
            ...
            if (userFormat !== undefined && userFormat !== pathFormat) {
                throw new ScreenshotSaveError('FORMAT_MISMATCH', `Path extension "${ext}" does not match format "${userFormat}"`, { source });
            }
            resolvedFormat = pathFormat;
            // Display check again with derived format
            if (source === 'display' && resolvedFormat === 'jpeg') {
                throw new ScreenshotSaveError('FORMAT_MISMATCH', 'Display screenshots only support PNG format. Cannot save as .jpg/.jpeg.', { source });
            }
        }
        else if (!ext) {
...
// ---------------------------------------------------------------------------
function validateMagicBytes(buffer, format) {
    if (format === 'png') {
...
function assertContainerScoped(client) {
    ...
    if (!hasTemplates) {
        throw new ScreenshotSaveError('CAPTURE_FAILED', 'saveScreenshot requires a container-scoped client. Call withContainer() first.');
    }
}
// ---------------------------------------------------------------------------
 ...
 * the wrong pty.
 */
function terminalCaptureTemplateVars(client, terminalId) {
...
async function captureTerminal(client, format, options) {
    const templateVars = options?.terminal_id !== undefined && options.terminal_id !== ''
...
// ---------------------------------------------------------------------------
async function saveScreenshotImpl(options) {
    assertContainerScoped(this);
    const { source, displayOptions, browserOptions, terminalOptions } = options;
    // Resolve path and format
    const { resolvedPath, resolvedFormat, needsMkdir, mkdirPath } = resolvePathAndFormat(source, options.path, options.dir, options.format);
    ...
    try {
        switch (source) {
            ...
            case 'terminal':
                ...
                break;
            default:
                throw new ScreenshotSaveError('CAPTURE_FAILED', `Unknown source: ${source}`);
        }
    }
    catch (err) {
        if (err instanceof ScreenshotSaveError)
            throw err;
        throw new ScreenshotSaveError('CAPTURE_FAILED', `Screenshot capture failed: ${err.message}`, {
            source,
            cause: err,
        });
    }
...
export function patchScreenshotSavePrototype(HoodyClientClass) {
