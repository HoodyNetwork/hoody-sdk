 ...
 */
function scoreCommand(tokens, group, cmd) {
    if (tokens.length === 0)
        return 0;
    let score = 0;
 ...
 */
function renderGroupIndex(reference) {
 ...
 * Main entry point.
 */
export function retrieveReference(opts) {
    ...
    const detailSection = selected.length > 0
        ? [
            '',
...
function collectFallbackCommands(reference) {
