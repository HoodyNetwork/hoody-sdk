...
export function titleFromUserMessage(firstUserContent) {
...
export async function createSession(opts) {
    ...
    let meta = buildMeta(opts, now, id);
...
function buildMeta(opts, now, id) {
    return {
        type: 'meta',
        id,
...
// ===== appendTurn =====
// ---------------------------------------------------------------------------
export async function appendTurn(filePath, turn) {
    await prepareChatsDir();
    const safe = {
        type: 'turn',
        role: turn.role,
...
export async function listSessions() {
    ...
    for (const name of entries) {
        ...
        if (!meta)
            continue; // readSession-side quarantine happens on real load
        results.push({
            id: meta.meta.id,
 ...
 */
async function readSessionMetaOnly(filePath) {
    let raw;
    let mtime = 0;
    try {
        raw = await readFile(filePath, 'utf-8');
 ...
 */
export async function findSessionById(idOrPrefix) {
 ...
 */
export async function findMatchingSessions(idOrPrefix) {
 ...
 * retried reply only.
 */
export async function truncateSessionTurns(filePath, keepCount) {
...
async function quarantine(filePath, reason) {
    ...
    try {
        ...
        process.stderr.write(`[hoody chat] Session file corrupted (${reason}), quarantined to ${quarantinePath}. Starting fresh.\n`);
    }
    catch {
        // If rename fails, there's nothing more we can do without risking
        // data loss — leave the file in place and warn.
        process.stderr.write(`[hoody chat] Session file corrupted (${reason}) but could not be quarantined: ${filePath}\n`);
    }
}
// ---------------------------------------------------------------------------
// ===== /save promotion =====
