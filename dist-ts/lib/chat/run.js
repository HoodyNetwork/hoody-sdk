...
export async function runChat(args) {
    ...
    // accept file entirely — no disk reads/writes.
    {
        ...
        if (provStatus.status === 'needs-tty-prompt') {
            ...
            const accepted = await new Promise(resolve => {
                rl.question(`hoody chat: Allow connecting to LLM provider at ${provStatus.origin}? [yes/no] `, answer => {
                    rl.close();
                    resolve(answer.trim().toLowerCase() === 'yes' || answer.trim().toLowerCase() === 'y');
                });
            });
            if (!accepted) {
                process.stderr.write(`hoody chat: LLM provider at ${provStatus.origin} not accepted. Exiting.\n`);
                process.exit(2);
            }
            if (!privateMode) {
            ...
            process.stderr.write(`hoody chat: LLM endpoint ${provStatus.origin} not accepted.\n` +
    ...
    if (!rawPrompt) {
        ...
        const toolsEnabled = args.opts.tools !== false && process.env.HOODY_CHAT_DOCS_TOOL !== '0';
    ...
    const toolsEnabled = args.opts.tools !== false && // Commander --no-tools flips this to false
    ...
    if (toolsEnabled) {
        ...
        if (trig.hit) {
            if (trig.query.length >= 8) {
                ...
                process.stderr.write('hoody chat: @hoody.com trigger detected but no query (< 8 chars) — skipping fetch.\n');
            }
        }
    }
    ...
    try {
        if (noStream) {
            ...
            if (toolsEnabled) {
                process.stderr.write('hoody chat: --no-stream disables the docs-search tool (tool_calls require streaming mode). ' +
...
function validateFiniteInt(flag, value, fallback, min, max) {
    if (value === undefined || value === null)
        return fallback;
    const n = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(n) || !Number.isInteger(n) || n < min || n > max) {
        process.stderr.write(`Error: --${flag} must be an integer in [${min}, ${max}] (got ${JSON.stringify(value)})\n`);
        process.exit(64);
    }
    return n;
}
/** Parse a float flag with range validation; exit 64 on bad input. */
function validateFiniteFloat(flag, value, fallback, min, max) {
    if (value === undefined || value === null)
        return fallback;
    const n = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(n) || n < min || n > max) {
        process.stderr.write(`Error: --${flag} must be a number in [${min}, ${max}] (got ${JSON.stringify(value)})\n`);
        process.exit(64);
    }
    return n;
}
