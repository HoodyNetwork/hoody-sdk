...
function stripTerminalControls(chunk) {
 ...
 */
function takeBytes(str, maxBytes) {
...
export async function askHoody(opts) {
    ...
    if (ok.status !== 'ok') {
        ...
        const remedy = acceptable && !declined && !alreadyAdvises
    ...
    while (true) {
        ...
        const result = await fetchOnce(url, message, history, maxBytes, totalMs, opts.firstByteTimeoutMs ?? DEFAULT_FIRST_BYTE_MS, fetchImpl, opts.onDelta, streamed, opts.signal);
        ...
        const retryAfterSec = result._retryAfterSec;
...
async function fetchOnce(url, message, history, maxBytes, totalMs, firstByteMs, fetchImpl, onDelta, streamed, callerSignal) {
    ...
    try {
        ...
        if (fatal)
            return { error: 'failed', message: stripTerminalControls(fatal) };
        if (framesSeen === 0) {
...
function mapHttpError(status, retryAfterHeader, body) {
    ...
    if (status === 408)
        return { error: 'timeout', message: 'The service timed out (408).' };
    if (status === 413) {
 ...
 */
async function abortableSleep(ms, sleep, signal) {
    if (!signal) {
        await sleep(ms);
        return true;
    }
