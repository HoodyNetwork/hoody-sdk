...
function getCacheFilePath(containerId) {
 ...
 */
export function readFileCache(containerId, ttlMs = DEFAULT_TTL_MS) {
    ...
    try {
        ...
        if (cache.containerId !== containerId)
            return undefined;
        const fetchedAt = new Date(cache.fetchedAt).getTime();
        if (Number.isNaN(fetchedAt))
            return undefined;
        const effectiveTtl = cache.ttlMs || ttlMs;
        if (Date.now() - fetchedAt > effectiveTtl)
            return undefined;
        return cache;
    }
    catch {
 ...
 */
export function writeFileCache(containerId, scripts, ttlMs = DEFAULT_TTL_MS) {
    ensureCacheDir();
    // Opportunistic prune — keeps the on-disk cache dir bounded without a
    // dedicated sweep task.
    pruneStaleCache();
 ...
 */
export function deleteFileCache(containerId) {
    const filePath = getCacheFilePath(containerId);
    try {
        if (existsSync(filePath))
            unlinkSync(filePath);
