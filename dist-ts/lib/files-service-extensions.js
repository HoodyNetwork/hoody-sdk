...
function patchFilesService(proto, includeJsonOverrides) {
    ...
    if (includeJsonOverrides) {
        ...
        if (typeof origList === 'function') {
            proto.listDirectory = function (...args) {
                const mutableArgs = [...args];
                const optionsIndex = 1;
                const options = (mutableArgs[optionsIndex] ?? {});
                if (!Object.prototype.hasOwnProperty.call(options, 'json')) {
...
function patchArchivesService(proto) {
    if (proto[ARCHIVES_PATCH_MARKER])
        return;
    proto.getDirectoryZipUrl = function (directory, templateVars) {
        if (!directory) {
...
function patchImageProcessingService(proto) {
