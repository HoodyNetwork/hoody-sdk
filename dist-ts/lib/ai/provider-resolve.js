...
const LOCAL_ORIGIN_REGEX = /^(https?):\/\/(localhost|127\.0\.0\.1|\[::1\]|10\.[0-9.]+|172\.(1[6-9]|2[0-9]|3[01])\.[0-9.]+|192\.168\.[0-9.]+)(:[0-9]+)?$/i;
/**
 ...
 */
function finalizeTier(cfg) {
    if (cfg.tier === 'openai') {
        if (!cfg.url) {
            return {
                error: 'openai-missing-url',
                message: 'OPENAI_API_KEY set but OPENAI_BASE_URL is missing. OPENAI_BASE_URL is REQUIRED for the OPENAI_* tier — no default is assumed to prevent accidental credential leak.',
                hint: 'Either set OPENAI_BASE_URL explicitly, or use HOODY_CHAT_* / HOODY_CLI_AI_* instead.',
            };
        }
        if (!cfg.model) {
            return {
                error: 'openai-missing-model',
                message: 'OPENAI_API_KEY set but OPENAI_MODEL is missing. OPENAI_MODEL is REQUIRED for the OPENAI_* tier.',
    ...
    try {
        ...
        return {
            error: 'bad-url',
            message: `Invalid URL for ${cfg.tier} tier: ${cfg.url} (${e.message})`,
        };
    }
 ...
 */
export function formatResolverError(err) {
