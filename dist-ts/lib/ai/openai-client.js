 ...
 */
export function resolveChatUrl(raw) {
    const u = new URL(raw);
 ...
 */
export function buildHeaders(opts, stream) {
    const h = new Headers({
        'Content-Type': 'application/json',
        Accept: stream ? 'text/event-stream' : 'application/json',
    });
    if (opts.key)
        h.set('Authorization', `Bearer ${opts.key}`);
    return h;
}
/**
...
export async function completeOnce(opts) {
    ...
    const timeout = setTimeout(() => controller.abort(new Error('timeout')), opts.timeoutMs ?? DEFAULT_ONESHOT_TIMEOUT_MS);
...
export async function streamCompletion(opts) {
    ...
    const controller = new AbortController();
    const totalTimeout = setTimeout(() => controller.abort(new Error('stream-timeout')), opts.timeoutMs ?? DEFAULT_STREAM_TIMEOUT_MS);
    ...
    let firstByteTimeout = setTimeout(() => controller.abort(new Error('first-byte-timeout')), opts.firstByteTimeoutMs ?? DEFAULT_FIRST_BYTE_MS);
    const callerAbort = opts.signal
        ? () => controller.abort(opts.signal.reason)
        : undefined;
    if (opts.signal && callerAbort) {
    ...
    try {
        ...
        for await (const frame of readSseFrames(res.body, opts.debug)) {
            ...
            try {
                ...
                if (opts.debug)
                    process.stderr.write(`[openai-client] Skipped malformed SSE frame: ${frame.slice(0, 100)}\n`);
                continue;
            }
            ...
            if (Array.isArray(delta.tool_calls)) {
                ...
                for (const raw of delta.tool_calls) {
                    ...
                    if (typeof r.index !== 'number' || !Number.isFinite(r.index))
                        continue;
                    const incoming = { index: r.index };
                    if (typeof r.id === 'string')
                        incoming.id = r.id;
                    if (typeof r.type === 'string')
                        incoming.type = r.type;
                    if (r.function && typeof r.function === 'object') {
                        ...
                        // as a tool-result error instead of throwing TypeError.
                            ...
                            try {
                                fn.arguments = JSON.stringify(f.arguments);
                            }
                            catch { /* drop */ }
                        }
 ...
 */
export async function* readSseFrames(body, debug = false) {
    ...
    try {
        ...
        if (debug)
            process.stderr.write('[openai-client] SSE stream closed\n');
        try {
            reader.releaseLock();
        }
