...
export function parseLocalTarget(input) {
    ...
    if (raw.includes('@')) {
        throw new Error(`Target must not contain userinfo. Credentials in URLs are not supported. Got: "${raw}"`);
    }
    // Pure number: "8080"
    if (/^\d+$/.test(raw)) {
    ...
    throw new Error(`Cannot parse target "${raw}". A port is required. Examples: 8080, localhost:3000, 10.0.0.1:8080`);
}
 ...
 */
export function parseContainerPort(input) {
