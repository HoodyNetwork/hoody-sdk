 ...
 */
export function mediaStreamToReadableStream(mediaStream, timeslice = 100, mimeType) {
...
export class PipeMedia {
     ...
     */
    static pickMimeType(mediaStream) {
    ...
    // Share screen
    // -------------------------------------------------------------------------
    async shareScreen(opts) {
        const displayOpts = opts?.displayMediaOptions
        ...
        if (streamOpts.viewer === undefined)
            streamOpts.viewer = true;
        return this.streamMediaToPipe(mediaStream, streamOpts);
    }
    // -------------------------------------------------------------------------
    // Share webcam
    // -------------------------------------------------------------------------
    async shareWebcam(opts) {
    ...
    // -------------------------------------------------------------------------
    async sendStream(path, stream, contentType) {
    ...
    // -------------------------------------------------------------------------
    async receiveMedia(path, videoElement, opts) {
    ...
    // -------------------------------------------------------------------------
    streamMediaToPipe(mediaStream, opts) {
        ...
        let mimeType;
        try {
            ({ stream, recorder, mimeType } = mediaStreamToReadableStream(mediaStream, timeslice, opts.mimeType));
        }
        catch (err) {
            for (const track of mediaStream.getTracks())
                track.stop();
        ...
        const shareParams = [];
        if (opts.n && opts.n > 1)
            shareParams.push(`n=${opts.n}`);
        if (opts.viewer)
            shareParams.push('video');
        if (shareParams.length > 0)
            shareUrl += '?' + shareParams.join('&');
        let stopped = false;
    ...
    // -------------------------------------------------------------------------
    receiveMediaDirect(path, videoElement, opts) {
        let stopped = false;
        let url = this.getUrl(path);
        if (opts?.n && opts.n > 1)
            url += `?n=${opts.n}`;
    ...
    // -------------------------------------------------------------------------
    async receiveMediaMSE(path, videoElement, opts) {
        ...
        if (!probeResult) {
            abortController.abort();
            throw new Error(`MSE cannot decode this stream. Tried: ${mimeVariants.join(', ')}. Use mode: 'direct'.`);
        }
        ...
        const done = new Promise((resolve, reject) => {
            ...
            mediaSource.addEventListener('sourceopen', async () => {
                ...
                const pendingChunks = [firstChunk];
                ...
                const tryEvict = () => {
                    try {
                        ...
                        if (bufferedEnd - bufferedStart < EVICT_BUFFER_SECONDS)
                            return false;
...
function parsePipeHeader(header, key) {
 ...
 */
function probeMimeType(initChunk, variants) {
 ...
 */
function buildMimeVariants(mimeType) {
    const variants = [mimeType];
...
function buildDisplayMediaOptions(opts) {
    const video = {};
    if (opts?.surfaceType)
        video.displaySurface = opts.surfaceType;
    if (opts?.cursor)
        video.cursor = opts.cursor;
    if (opts?.frameRate)
        video.frameRate = { ideal: opts.frameRate };
    if (opts?.width)
        video.width = { ideal: opts.width };
    if (opts?.height)
        video.height = { ideal: opts.height };
    const result = {
    ...
    if (opts?.systemAudio)
        result.systemAudio = opts.systemAudio;
...
function buildStreamOpts(opts) {
    const result = {};
    if (opts?.path !== undefined)
        result.path = opts.path;
    if (opts?.timeslice !== undefined)
        result.timeslice = opts.timeslice;
    if (opts?.mimeType !== undefined)
        result.mimeType = opts.mimeType;
    if (opts?.n !== undefined)
        result.n = opts.n;
    if (opts?.viewer !== undefined)
        result.viewer = opts.viewer;
    return result;
}
