 ...
 */
export function cacheDir(platform = process.platform, env = process.env) {
...
export function cachePath(platform = process.platform, env = process.env) {
 ...
 */
export function readCache(customPath) {
    ...
    try {
        ...
        // decisions.
        const validStatuses = ['up-to-date', 'behind', 'ahead', 'error'];
        if (!validStatuses.includes(parsed.status))
            return null;
        if (parsed.latest_version !== null &&
            typeof parsed.latest_version !== 'string')
            return null;
        if (parsed.error !== null && typeof parsed.error !== 'string')
            return null;
        if (parsed.not_after !== null && typeof parsed.not_after !== 'string')
            return null;
        if (typeof parsed.ttl_seconds !== 'number' ||
 ...
 */
export async function writeCache(data, customPath) {
    const filePath = customPath ?? cachePath();
...
export function cleanupStaleTmpFiles(customDir, maxAgeMs = TMP_CLEANUP_MAX_AGE_MS, nowMs = Date.now()) {
...
export function isFresh(cache, nowMs = Date.now(), ttlSeconds = cache.ttl_seconds ?? DEFAULT_TTL_SECONDS) {
