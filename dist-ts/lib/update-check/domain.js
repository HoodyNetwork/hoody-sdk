...
function validateDomain(raw) {
    ...
    if (!lower)
        throw new DomainInvalidError('domain is empty');
    if (lower.length > 253)
        throw new DomainInvalidError(`domain exceeds 253 chars: ${lower}`);
    const labelRe = /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/;
    for (const label of lower.split('.')) {
        if (!labelRe.test(label))
            throw new DomainInvalidError(`invalid label ${JSON.stringify(label)} in ${lower}`);
    }
    return lower;
}
...
export function configFilePath(opts = {}) {
...
export function fingerprintForPubkey(pubkeyBase64) {
    return crypto.createHash('sha256').update(Buffer.from(pubkeyBase64.trim(), 'base64')).digest('hex');
}
export function verifyConfigFingerprint(domainConfigPath, bakedPubkeyBase64) {
    ...
    try {
        ...
        const err = e;
        if (err.code === 'ENOENT')
            return { status: 'absent', expected, actual: null };
 ...
 */
export function lookupTrustedDomain(map, domain) {
    if (!Object.prototype.hasOwnProperty.call(map, domain)) {
        const trusted = Object.keys(map).sort().join(', ');
        throw new DomainNotConfiguredError(`binary does not trust domain ${JSON.stringify(domain)}; trusted domains: ${trusted || '(none)'}`);
    }
    return map[domain];
}
/**
 ...
 */
export function selectDomain(opts = {}) {
    const env = opts.env ?? process.env;
    // 1. CLI flag.
    ...
    if (fileDomain)
        return fileDomain;
    // 4. Fail-closed.
    throw new DomainNotConfiguredError(`no domain configured (no --domain flag, no HOODY_DOMAIN env, no ${filePath}). ` +
