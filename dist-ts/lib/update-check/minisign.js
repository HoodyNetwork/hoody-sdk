...
export function parsePublicKey(input) {
    ...
    if (bytes.length !== PUBKEY_LENGTH) {
        throw new MinisignVerificationError(`pubkey record must be ${PUBKEY_LENGTH} bytes, got ${bytes.length}`);
    }
    if (bytes[0] !== PUBKEY_ALGO[0] || bytes[1] !== PUBKEY_ALGO[1]) {
        throw new MinisignVerificationError(`pubkey algo bytes must be "Ed" (0x45 0x64), got 0x${toHex(bytes[0])} 0x${toHex(bytes[1])}`);
    }
    return {
...
export function parseSignature(input) {
    ...
    if (lines.length !== 4) {
        throw new MinisignVerificationError(`signature file must have exactly 4 non-empty lines, got ${lines.length}`);
    }
    const [untrustedLine, mainB64, trustedLine, globalB64] = lines;
    if (!/^untrusted comment:/i.test(untrustedLine)) {
        throw new MinisignVerificationError(`line 1 must start with "untrusted comment:", got: ${untrustedLine.slice(0, 40)}`);
    }
    if (!/^trusted comment:/i.test(trustedLine)) {
        throw new MinisignVerificationError(`line 3 must start with "trusted comment:", got: ${trustedLine.slice(0, 40)}`);
    }
    const mainRecord = tryBase64Decode(mainB64, 'main signature record');
    if (mainRecord.length !== MAIN_RECORD_LENGTH) {
        throw new MinisignVerificationError(`main record must be ${MAIN_RECORD_LENGTH} bytes, got ${mainRecord.length}`);
    }
    ...
    if (mainRecord[0] === SIG_ALGO_LEGACY_PURE[0] && mainRecord[1] === SIG_ALGO_LEGACY_PURE[1]) {
        throw new MinisignVerificationError(`signature uses legacy pure Ed25519 ("Ed"); only prehashed ("ED") is accepted`);
    }
    if (mainRecord[0] !== SIG_ALGO_PREHASHED[0] || mainRecord[1] !== SIG_ALGO_PREHASHED[1]) {
        throw new MinisignVerificationError(`signature algo bytes must be "ED" (0x45 0x44), got 0x${toHex(mainRecord[0])} 0x${toHex(mainRecord[1])}`);
    }
    const globalSig = tryBase64Decode(globalB64, 'global signature');
    if (globalSig.length !== GLOBAL_SIG_LENGTH) {
        throw new MinisignVerificationError(`global signature must be ${GLOBAL_SIG_LENGTH} bytes, got ${globalSig.length}`);
    }
 ...
 */
export function verifyMessage(message, sig, pub) {
    // 1. Key-ID must match between signature and pubkey.
    if (!bytesEqual(sig.keyId, pub.keyId)) {
        throw new MinisignVerificationError(`key-id mismatch: sig=${toHexBytes(sig.keyId)} pub=${toHexBytes(pub.keyId)}`);
    }
 ...
 */
export function extractVersionFromTrustedComment(trustedComment) {
    const match = trustedComment.match(/\bversion=(\S+)/);
    if (!match) {
        throw new MinisignVerificationError(`trusted comment does not contain "version=X.Y.Z": ${JSON.stringify(trustedComment)}`);
    }
    return match[1];
}
/**
 ...
 */
export function validateFreshness(issuedAtIso, notAfterIso, nowMs = Date.now()) {
    ...
    if (!issuedClean)
        throw new MinisignVerificationError('channel.json missing issued_at');
    if (!notAfterClean)
        throw new MinisignVerificationError('channel.json missing not_after');
    ...
    if (issuedAt > nowMs + ISSUED_AT_FUTURE_SKEW_SECONDS * 1000) {
        throw new MinisignVerificationError(`issued_at is in the future (now=${new Date(nowMs).toISOString()}, issued_at=${issuedAtIso})`);
    }
    if (notAfter <= issuedAt) {
        throw new MinisignVerificationError(`not_after (${notAfterIso}) must be strictly after issued_at (${issuedAtIso})`);
    }
    if (nowMs > notAfter) {
        throw new MinisignVerificationError(`channel.json expired (not_after=${notAfterIso}, now=${new Date(nowMs).toISOString()})`);
    }
}
// ─── internal helpers ───────────────────────────────────────────────────────
function extractBase64Line(input, skipPattern) {
...
function tryBase64Decode(s, label) {
    ...
    try {
        ...
        throw new MinisignVerificationError(`${label} is not valid base64: ${e.message}`);
    }
}
