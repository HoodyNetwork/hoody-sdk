 ...
 */
export async function fetchOnce(url, timeoutMs, opts = {}) {
    ...
    try {
        ...
        if (cl && Number(cl) > MAX_RESPONSE_BYTES) {
            throw new FetchError(`response too large (declared ${cl} bytes, cap ${MAX_RESPONSE_BYTES}): ${url}`, 'network', url);
        }
        ...
        if (err.name === 'AbortError') {
            ...
            if (externalAborted) {
                throw new FetchError(`fetch cancelled by caller: ${url}`, 'external-abort', url);
            }
        ...
        const code = err.code ?? e.cause?.code;
...
export function isRetryable(err) {
    if (err.kind === 'external-abort')
        return false; // caller cancelled — respect it
    if (err.kind === 'abort')
        return true; // our own timeout — retry
    if (err.kind === 'network') {
        if (err.errno === 'ENOTFOUND')
            return false; // DNS NXDOMAIN = permanent
        return true;
    }
    // HTTP
...
async function readCapped(response, url) {
    if (!response.body) {
        ...
        if (ab.byteLength > MAX_RESPONSE_BYTES) {
            throw new FetchError(`response too large (${ab.byteLength} bytes, cap ${MAX_RESPONSE_BYTES}): ${url}`, 'network', url);
        }
    ...
    while (true) {
        ...
        if (total > MAX_RESPONSE_BYTES) {
            ...
            throw new FetchError(`response too large (>${MAX_RESPONSE_BYTES} bytes): ${url}`, 'network', url);
        }
        chunks.push(value);
    }
